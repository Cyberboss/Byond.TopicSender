...
namespace Byond.TopicSender
{
	...
	public interface IByondTopicSender
	{
		...
		Task<string?> SendTopic(string destinationServer, string queryString, ushort port, CancellationToken cancellationToken = default);
		...
		/// <param name="cancellationToken">The <see cref="CancellationToken"/> for the operation</param>
		/// <returns>A <see cref="Task{TResult}"/> resulting in the returned <see cref="string"/> from the server</returns>
		Task<string?> SendTopic(IPAddress address, string queryString, ushort port, CancellationToken cancellationToken = default);
